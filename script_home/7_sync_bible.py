"""
7_sync_bible.py — 從 bible/_index.md 同步連結文字到各頁面的 title
================================================================
用法：
    cd C:\\Users\\erich\\Desktop\\my-portfolio
    python script_home/7_sync_bible.py

功能：
    讀取 content/bible/_index.md 裡每個連結的文字，
    自動更新對應 .md 檔案的 title: front matter。

    只改 title: 那一行，所有內文完全不動。

日常工作流程：
    1. 打開 content/bible/_index.md，修改連結文字
       例：[舊約聖經《摩西五經》《創世記》《1》01~06]
       改成：[創世記｜起初神創造（1-6章）]
    2. 執行：python script_home/7_sync_bible.py
    3. 對應的 genesis-01.md 的 title: 就會自動更新
    4. git add + commit + push

注意：
    第一次使用前請先執行 6_update_bible_index.py
    讓 _index.md 的連結格式正確。
"""
import re
...
def main():
    ...
    # 找出所有 [連結文字](/bible/slug/) 配對
    pattern = r'\[([^\]]+)\]\(/bible/([^/]+)/\)'
    matches = re.findall(pattern, index_text)

    if not matches:
        print("bible/_index.md 裡沒有找到任何連結，請確認格式是否正確。")
        return
